-- Managers can see all the records of employees working under their coordination/Management.
------------------------------context-------------------------------------------
...
BEGIN
    ...
    IF (job_id='FI_MGR' OR job_id='AC_MGR' OR job_id ='SA_MAN' OR job_id='PU_MAN' OR job_id='ST_MAN' OR job_id='MK_MAN')
    THEN
        auth_pred := '"EMPLOYEE_ID"=SYS_CONTEXT(''test_AUTH_details'', ''emp_no'') OR "MANAGER_ID"=SYS_CONTEXT(''test_AUTH_details'', ''emp_no'')';        
    ELSE
...
------------------------------policy--------------------------------------------
BEGIN 
DBMS_RLS.DROP_POLICY('hr', 'EMPLOYEES', 'TEST_AUTH_POLICY');   --drop policy
END;
...
create user Michael identified by pwd;
GRANT CONNECT,RESOURCE TO Michael;
GRANT SELECT ON "HR"."EMPLOYEES" TO Michael;